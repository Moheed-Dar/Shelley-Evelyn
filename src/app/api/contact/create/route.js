...
export async function POST(request) {
  try {
    ...
    // ✅ Required Fields Check - Ab sirf Name aur Message required hain
    if (!name || !message) {
      return NextResponse.json(
    ...
    const newContact = await Contact.create({
      name: name.trim(),
      // ✅ Agar email hai toh trim/lowercase karo, warna undefined rehne dein
    ...
    sendContactEmail({
      name: newContact.name,
      email: newContact.email || 'Not provided',
      phone: newContact.phone || 'Not provided',
    ...
    // ✅ 2. User Ko Thank You Email Tabhi Bhejo Jab Email Di Ho
    if (newContact.email) {
      sendLeadThankYouEmail(newContact).catch((error) => {
        console.error("User thank you email fail:", error.message);
      });
    ...
    // User Ko Fataak Se Success Response Bhejo
    return NextResponse.json(
      { 
        success: true, 
        message: "Your message has been sent successfully! We will get back to you soon.", 
